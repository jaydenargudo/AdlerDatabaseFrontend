...
void createBindings(webview::webview &w) {
  ...
  w.bind("getMemberExportData", [&](const std::string &msg) -> std::string {
    ...
    json membersJSON = json::parse(result);
    for (const auto &member : membersJSON["id"]) {
      try {
        std::string memberId = member;

        std::future<std::string> memberFuture =
            std::async(std::launch::async, [memberId]() {
              json payload;
              payload["id"] = memberId;
              return APIClient::get_member(payload);
            });

        std::string memberDataString = memberFuture.get();
        json memberData = json::parse(memberDataString);
        LogToFile(memberData.dump(4));

        std::future<std::string> enrollmentFuture, medicalFuture,
            transportFuture;

        if (member.contains("enrollment_form") &&
            member["enrollment_form"] != "None" &&
            member["enrollment_form"].size() > 0) {
          enrollmentFuture = std::async(std::launch::async, [member]() {
            json payload;
            payload["id"] = member["enrollment_form"][0];
            return APIClient::get_membership_enrollment_form(payload);
          });
        }
        if (member.contains("medical_history") &&
            member["medical_history"] != "None" &&
            member["medical_history"].size() > 0) {
          medicalFuture = std::async(std::launch::async, [member]() {
            json payload;
            payload["id"] = member["medical_history"][0];
            return APIClient::get_medical_history_form(payload);
          });
        }
        if (member.contains("transport_info") &&
            member["transport_info"] != "None" &&
            member["transport_info"].size() > 0) {
          transportFuture = std::async(std::launch::async, [member]() {
            json payload;
            payload["id"] = member["transport_info"][0];
            return APIClient::get_transportation_information(payload);
          });
        }

        // Gather results (default to "None" if not set)
        std::string enrollmentData =
            (enrollmentFuture.valid()) ? enrollmentFuture.get() : "None";
        std::string medicalData =
            (medicalFuture.valid()) ? medicalFuture.get() : "None";
        std::string transportData =
            (transportFuture.valid()) ? transportFuture.get() : "None";

        json memberFullData;
        memberFullData["userData"] = memberData;
        memberFullData["enrollmentData"] = (enrollmentData == "None")
                                               ? json::object()
                                               : json::parse(enrollmentData);
        memberFullData["medicalData"] =
            (medicalData == "None") ? json::object() : json::parse(medicalData);
        memberFullData["transportData"] = (transportData == "None")
                                              ? json::object()
                                              : json::parse(transportData);

        finalJSON[memberId] = memberFullData;
      } catch (const std::exception &e) {
        LogToFile(std::string("Exception while processing member: ") +
                  e.what());
      } catch (...) {
        LogToFile("Unknown error occurred while processing member.");
      }
    }
    LogToFile("Member data exported successfully");
  ...
  w.bind("loadMemberForm", [&](const std::string &msg) -> std::string {
    ...
    if (msg != "") {
      try {
        ...
        std::future<std::string> getTransportData;
        ...
        if ((userJSON["enrollment_form"] != "None") &&
            (userJSON["enrollment_form"].size() > 0)) {
        ...
        if ((userJSON["medical_history"] != "None") &&
            (userJSON["medical_history"].size() > 0)) {
        ...
        if ((userJSON["transport_info"] != "None") &&
            (userJSON["transport_info"].size() > 0)) {
          getTransportData = std::async(std::launch::async, [=]() {
            json localPayload = jsonPayload;
        ...
        if ((userJSON["emergency_contact_one"] != "None") &&
            (userJSON["emergency_contact_one"].size() > 0)) {
        ...
        if ((userJSON["emergency_contact_two"] != "None") &&
            (userJSON["emergency_contact_two"].size() > 0)) {
        ...
        std::string transportData = "None";
        std::string ec1Data = "None";
        std::string ec2Data = "None";

        if (getEnrollmentData.valid())
          enrollmentData = getEnrollmentData.get();
        if (getMedicalData.valid())
          medicalData = getMedicalData.get();
        if (getTransportData.valid())
          transportData = getTransportData.get();
        if (getEmergencyOne.valid())
          ec1Data = getEmergencyOne.get();
        if (getEmergencyTwo.valid())
          ec2Data = getEmergencyTwo.get();
        ...
        combinedData["transportData"] = json::parse(transportData);
        ...
        LogToFile(
            "Unknown error occurred while getting and processing member data.");
      }
    }
    return "";
  });
  ...
  w.bind("loadContactForm", [&](const std::string &msg) -> std::string {
    ...
    if (msg != "") {
      try {
        ...
        LogToFile("Unknown error occurred while getting and processing contact "
                  "data.");
      }
    }
    return "";
  ...
  w.bind("createMemberModal", [&](const std::string &msg) -> std::string {
    ...
    std::string userData = APIClient::get_member(jsonPayload);
    json userJSON = json::parse(userData);
    auto getEnrollmentData =
        std::async(std::launch::async, [=]() { // Capture by value
    ...
    safeEnrollmentData =
        safeEnrollmentData.substr(1, safeEnrollmentData.length() - 2);
    ...
    w.eval("memberModal(`" + userData + "`,`" + safeEnrollmentData + "`,`" +
           safeMedicalData + "`,`" + transportData + "`,`" + emergency_one +
           +"`,`" + emergency_two + "`,`" + caregivers + "`);");
    return "";
  });
  ...
  w.bind("createCaregiverModal", [&](const std::string &msg) -> std::string {
    ...
    std::string safeMedicalData = escape_json_for_js(medicalData);
    safeMedicalData = safeMedicalData.substr(1, safeMedicalData.length() - 2);
    w.eval("caregiverModal(`" + userData + "`,`" + safeMedicalData + "`,`" +
           emergency_one + +"`,`" + emergency_two + "`);");
    return "";
  });
  ...
  w.bind("addEnrollmentForm", [&](const std::string &msg) -> std::string {
    json jsonPayload = json::parse(msg);
    std::string result =
        APIClient::create_membership_enrollment_form(jsonPayload);
    return result;
  });
  ...
  w.bind("addTransportForm", [&](const std::string &msg) -> std::string {
    json jsonPayload = json::parse(msg);
    std::string result =
        APIClient::create_transportation_information(jsonPayload);
    return result;
  });
